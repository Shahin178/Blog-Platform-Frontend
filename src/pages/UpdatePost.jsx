...
const UpdatePost = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const { data } = await api.get(`/blog/post/${postId}`);
        setForm({
  ...
  if (!form) {
    return (
      <div className="flex items-center justify-center min-h-[calc(93vh-64px)]">
        <span className="text-gray-500 animate-pulse">Loading post...</span>
      </div>
    );
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.put(`/blog/updatePost/${postId}`, {
        title: form.title,
    ...
    <div className="flex items-center justify-center min-h-[calc(93vh-64px)] bg-gradient-to-br from-blue-50 to-purple-100">
      ...
      >
        ...
        <div className="space-y-4">
          ...
          <CKEditor
            key={form.content} // forces re-render when content changes
